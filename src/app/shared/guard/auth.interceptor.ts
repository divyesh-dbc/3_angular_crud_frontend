...
export class MyInterceptor implements HttpInterceptor {
  constructor(private Auth: AuthService, private loader: LoaderService, public router: Router) { }

  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    const x = window.scrollX;
    const y = window.scrollY;
    const routPath = this.router.url.split("?")[0];
    console.log("routPath", routPath);
      ...
      return next.handle(reqHeader).pipe(
        ...
        catchError((err: any) => {
          if (err instanceof HttpErrorResponse) {
            ...
            window.onscroll = function () { };
            this.loader.setLoading(false);
            if (err.status === 401 && routPath != "/resetpassword" &&routPath != "/verifyuser"&&routPath != "/forgotpassword" ) {
              this.Auth.logOut();
            }
            return throwError(err);
          }
